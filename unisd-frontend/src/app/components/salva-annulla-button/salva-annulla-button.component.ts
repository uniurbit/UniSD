...
@Component({
  ...
  <div class="form-footer text-right" *ngIf="formRef">
    <button  type="button" *ngIf="!isNew" class="btn btn-outline-secondary rounded mr-1"  
      (click)="annulla()" title="{{ 'btn_annulla_title' | translate }}" >{{ 'btn_annulla' | translate }}</button>        
    <button  type="submit" class="btn btn-outline-secondary rounded mr-1"  [disabled]="disabled()" 
