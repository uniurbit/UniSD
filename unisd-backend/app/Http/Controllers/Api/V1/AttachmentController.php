...
class AttachmentController extends Controller
{
    ...
    public function download($id){
        ...
        if ($attach->attachmenttype_codice == 'DOMANDA'){
            if (!Auth::user()->hasPermissionTo('search all domande')){
                abort(403, trans('global.utente_non_autorizzato'));
            }
        }

        if ($attach->attachmenttype_codice == 'BANDO_ALLEGATO_PRIVATO'){
            if (!(Auth::user()->hasRole('super-admin')||Auth::user()->hasRole('op_uff_amm'))){
                abort(403, trans('global.utente_non_autorizzato'));
            }
        }
