...
class QueryBuilder
{
    ...
    private function setWhere($where){
        if ($this->isRelationColumn($where->field) && !($where->type && $where->type=="alias")){
    ...
    {        
       ...
       if ($where->operator == 'In') {
            ...
            if ( $where->value === '[null]') {
                ...
                if ($where->type == 'date'){
                    $query->whereDate($where->field, $where->operator, $where->value);
                } else{
    ...
    {        
        ...
        /** @var string $type */
            ...
            $this->query->where(function ($query) use($where){
                $query->where($where->field, '!=', $where->value)->orWhereNull($where->field);
            });
        } else {
            if ( $where->value === '[null]') {
                if ( $where->operator == '=') {
                ...
                if ($where->type == 'date'){
                    $this->query->whereDate($where->field, $where->operator, $where->value);
                }else{
    ...
    private function basePaginate($perPage = null, $columns = ['*'], $pageName = 'page', $page = null)
    {
        ...
        return (new Paginator($results, $total, $perPage, $page, [
            ...
            'pageName' => $pageName            
        ])); //->setQueryUri($this->request->getRequestUri());
    }
