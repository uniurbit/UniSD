...

{
    ...
    protected $hidden = ['model_type', 'model_id','key','group','metadata','title','preview_url'];
    ...
    {
        //non esegue la query correttamente        
        $myroles = User::with(['roles'])->where('id',Auth::user()->id)->first()->roles;        
        if ($myroles->contains('name', 'super-admin') || $myroles->contains('name', 'op_uff_amm')){
            return $query->with('attachmenttype');            
        }

        //SE UTENTE è DOCENTE solo documenti pubblici
        //op_commissione
        return $query->whereIn('attachmenttype_codice',['BANDO_ALLEGATO','DOMANDA','DOMANDA_BOZZA'])->with('attachmenttype');
        
    }
    
    public function scopeAttachmentTypePrivate($query)
    {
        return $query->where('attachmenttype_codice','BANDO_ALLEGATO_PRIVATO')->with('attachmenttype');
    }
